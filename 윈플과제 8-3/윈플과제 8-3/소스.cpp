	...
	LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_FILESAVE:
			...
			for (int i = 0; i < 5; ++i)
			{
				sprintf(outBuff, "%d %d%d %d%d %d%d %d%d%d %d%d%d %d%d%d ", shape[i].shape, shape[i].X / 10, shape[i].X % 10, shape[i].Y / 10, shape[i].Y % 10, shape[i].n / 10, shape[i].n % 10, shape[i].r / 100, (shape[i].r / 10) % 10, shape[i].r % 10, shape[i].g / 100, (shape[i].g / 10) % 10, shape[i].g % 10, shape[i].b / 100, (shape[i].b / 10) % 10, shape[i].b % 10);
		...
		case ID_FILELOAD:
			...
			for (int i = 0; i < 5; ++i)
			{
				shape[i].shape = (int)inBuff[n] - 48;
				n += 2;
				shape[i].X = ((int)inBuff[n] - 48) * 10 + ((int)inBuff[n + 1] - 48);
				n += 3;
				shape[i].Y = ((int)inBuff[n] - 48) * 10 + ((int)inBuff[n + 1] - 48);
				n += 3;
				shape[i].n = ((int)inBuff[n] - 48) * 10 + ((int)inBuff[n + 1] - 48);
				n += 3;
				shape[i].r = ((int)inBuff[n] - 48) * 100 + ((int)inBuff[n + 1] - 48) * 10 + ((int)inBuff[n + 2] - 48);
				n += 4;
				shape[i].g = ((int)inBuff[n] - 48) * 100 + ((int)inBuff[n + 1] - 48) * 10 + ((int)inBuff[n + 2] - 48);
				n += 4;
				shape[i].b = ((int)inBuff[n] - 48) * 100 + ((int)inBuff[n + 1] - 48) * 10 + ((int)inBuff[n + 2] - 48);
				n += 4;
			}
			CloseHandle(hFile);
